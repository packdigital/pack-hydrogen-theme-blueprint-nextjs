 ...
 *
 * Note: Hydrogen's `handleShopifyRoutes` (in `src/proxy.ts`) already handles
 * unprefixed numeric permalinks such as `/cart/123:1` by redirecting to
 * Shopify's cart permalink. This handler serves the remaining forms and
 * locale-prefixed URLs (formerly `routes/($locale).cart.$lines.tsx`).
 *
...
type Params = {locale?: string; lines: string};
...
async function loader({request, context, params}: LoaderArgs<Params>) {
  ...
  if ((result.errors as unknown[] | undefined)?.length || !cartResult) {
