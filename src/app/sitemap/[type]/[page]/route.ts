...
import type {AppLoadContext} from '~/lib/server/context';
import {routeHandler} from '~/lib/server/route';
import type {LoaderArgs} from '~/lib/server/route';
import {getSitemap, PACK_NATIVE_TEMPLATE_TYPES} from '~/lib/server/sitemap';
...
async function loader({
  ...
  params: rawParams,
  context,
}: LoaderArgs<{type: string; page: string}>) {
  // App Router receives the `[page]` segment with its `.xml` suffix (`1.xml`)
  const params = {
    ...rawParams,
    page: rawParams.page?.replace(/\.xml$/, ''),
  };
