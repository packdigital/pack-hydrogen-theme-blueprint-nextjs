...
export const ProductStickyAddToCart = forwardRef(
  (
    ...
    addToCartRef: React.ForwardedRef<HTMLDivElement>,
  ) => {
    ...
    useEffect(() => {
      const scrollHandler = () => {
        const addToCartEl =
          addToCartRef && typeof addToCartRef === 'object'
            ? addToCartRef.current
            : null;
        const addToCartPosition = addToCartEl?.getBoundingClientRect().top || 0;
        const addToCartHeight = addToCartEl?.clientHeight || 0;
