...
Shopify's Graphql Admin API can now be utilized with this client within each loader and action `context` (from `~/lib/server/context`) — alongside `storefront` and `pack` - under the name of `admin`.
...
The Admin API client `admin` is modeled after the `storefront` client in `~/lib/server/storefront` and can be used similarly, except there is no support for using the public API token for Admin API. It is available on the server context returned by `getContext()` (server components) and `getRouteContext()` (route handlers), and caches through Next's Data Cache.
