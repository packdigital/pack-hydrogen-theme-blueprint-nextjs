...
import type {OxygenEnv} from './oxygen.server';

export function getLocaleFromRequest(request: {url: string}): I18nLocale {
...
export const REDIRECTED_TO_LOCALE_COOKIE = 'redirected_to_locale';
...
export async function redirectLinkToBuyerLocale({
  ...
  context: {storefront: Storefront; oxygen: OxygenEnv};
  request: Request;
}): Promise<
